...
def rewrite_runestone_swift(text: str, relative: Path | None = None) -> str:
    ...
    if name == "TreeSitterParser.swift":
...
def header_search_paths(src_root: Path, language_names: list[str]) -> list[str]:
    paths = [
        "$(SRCROOT)/build/src/public",
        "$(SRCROOT)/build/src/TreeSitterCore/include",
        "$(SRCROOT)/build/src/TreeSitterCore/src",
    ]
    for name in language_names:
        paths.append(f"$(SRCROOT)/build/src/Languages/{name}/src")
        paths.append(f"$(SRCROOT)/build/src/Languages/{name}/include")
    return paths
...
def write_xcode_project(root: Path, language_names: list[str]) -> None:
    ...
    header_paths = header_search_paths(root, language_names)
    ...
    def settings_block(extra: dict[str, str], indent: str = "\t\t\t\t") -> str:
        merged = {
            ...
            "SWIFT_INCLUDE_PATHS": '"$(SRCROOT)/build/src/clang"',
...
/* Begin PBXFileReference section */
		...
		{queries_ref_id} /* Queries */ = {{isa = PBXFileReference; lastKnownFileType = folder; name = Queries; path = build/src/Runestone/Resources/Queries; sourceTree = "<group>"; }};
...
"""
    ...
    project_dir = root / "Runestone.xcodeproj"
